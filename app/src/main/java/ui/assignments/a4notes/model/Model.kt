...
class Model {
     ...
     *  @param timestamp indicates the last-edit date of the note
     */
    data class ModelNote(val id: Int, var title: String, var content: String, var important : Boolean = false, var archived : Boolean = false) : BaseObservable() {
        var timestamp = GregorianCalendar.getInstance().time.time
    }
    ...
    fun updateNoteTimestamp(id: Int) {
        notes.find { it.id == id }.apply {
            this?.timestamp = GregorianCalendar.getInstance().time.time
            this?.notifyPropertyChanged(id)
        }
    }

    /**
     * Compares two notes. The order is: newer edited notes come first.
    ...
    fun compareNotes(idA: Int, idB: Int) : Int {
        ...
        return if (b == null) -1
                    ...
                    sign((b.timestamp - a.timestamp).toDouble()).toInt()
        }
    }
