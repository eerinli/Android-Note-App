...
class ViewGrid : Pane(), InvalidationListener {
    ...
    init {
        val transitionFade = FadeTransition(Duration.seconds(1.0), startText)
        transitionFade.fromValue = 1.0
        transitionFade.toValue = 0.0

        val transitionFade2 = FadeTransition(Duration.seconds(1.0), startButton)
        transitionFade2.fromValue = 1.0
        transitionFade2.toValue = 0.0

        startButton.addEventFilter(MouseEvent.MOUSE_CLICKED) {
            transitionFade.play()
            transitionFade2.play()
            Model.startGame()
        }
