...
internal class ToolbarView(
    ...
    private val desktopButton = Button("Desktop").apply {
        prefWidth = 70.0
    }
    ...
    init {
        ...
        listButton.addEventHandler(MouseEvent.MOUSE_CLICKED) {
            model.changeViewSelection(0)
        }
        // register the controller as a handler for the "Grid" button
        gridButton.addEventHandler(MouseEvent.MOUSE_CLICKED) {
            model.changeViewSelection(1)
        }
        // register the controller as a handler for the "Desktop" button
        desktopButton.addEventHandler(MouseEvent.MOUSE_CLICKED) {
            model.changeViewSelection(2)
        }

        val viewGroup = HBox(viewLabel, listButton, gridButton, desktopButton, Separator(Orientation.VERTICAL)).apply {
        ...
        val sortChoice = ChoiceBox(FXCollections.observableArrayList("Length (asc)", "Length (desc)",
            "Note Content (asc)", "Note Content (desc)", "Created Time (asc)", "Created Time (desc)" ))
